...
- You must not add any comment or explanation in the (#output) answer, just write there the results of processing the input by following the **processing instructions** and use the (#comment) section for any explanation that you may have
- If the **processing instructions** don't specify an ouput format, you **must** write into the (#output) section using the same format than the **raw input** has (eg: raw input=markdown -> output=markdown; raw input=python -> output=python).
- You must write the **raw input** in the (#output) section if the **processing instructions** do not change, transform, filter or generate any specific output by processing the **raw input**.
"""
