...
#     if len(lines) > 1 and lines[0] == "(#context)":
#         i=1
#         prev_context = ""
#         while (i<len(lines) and lines[i] not in ["(#output)","#(error)"]):
#             prev_context += lines[i] + "\n"
#             i+=1
#         lines = lines[i:]
#         log.info("Info: We got some context!!!: " + prev_context)
# 
# 
#     if len(lines) > 1 and lines[0] == "(#output)":
#         print("\n".join(lines[1:]))
